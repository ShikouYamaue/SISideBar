...
class WeightCopyPaste:
    def main(
        ...
        if path == "default":
            ...
            self.scene_path = os.sep.join(
                cmds.file(q=True, sceneName=True).split(os.sep)[:-1]
            )
            self.protect_path = os.path.join(
                self.scene_path, "weight_protector"
            )
            try:
    ...
    def weightPaste(self):
        ...
        for skinMesh in self.skinMeshes:
            ...
            dstSkinCluster = cmds.ls(
                cmds.listHistory(skinMesh), type="skinCluster"
            )
            ...
            if not dstSkinCluster:
                ...
                if os.path.exists(self.fileName):
                    try:
                        with open(
                            self.fileName, "r"
                        ) as f:  # ファイル開く'r'読み込みモード'w'書き込みモード
                            ...
                            maintainMaxInfluences = saveData[
                        ...
                        common.TemporaryReparent().main(
                            skinMesh, dummyParent=dummy, mode="cut"
                        )
                        ...
                        # 親子付けを戻す
                        common.TemporaryReparent().main(
                            skinMesh, dummyParent=dummy, mode="parent"
                        )
            ...
            if self.engine == "maya":
                ...
                if os.path.isfile(self.filePath + os.sep + xml_name):
                    ...
                    cmds.skinCluster(
                        dstSkinCluster, e=True, forceNormalizeWeights=True
                    )
    ...
    def weightCopy(self):
        ...
        if not os.path.exists(self.filePath):
            os.makedirs(
                os.path.dirname(self.filePath + os.sep)
            )  # 末尾\\が必要なので注意
        else:  # ある場合は中身を削除
            files = os.listdir(self.filePath)
...
def transfer_weight(
    ...
    massege01 = lang.Lang(
        ...
        ja=": スキンメッシュではないのでウェイトの転送を行いません",
    ).output()
    massege02 = lang.Lang(en="Transfer the weight:", ja="ウェイトを転送:").output()
    massege03 = lang.Lang(
        en="Transfer bind influences:", ja="バインド状態を転送:"
    ).output()
    ...
    influences = cmds.skinCluster(
        srcSkinCluster, q=True, inf=True
    )  # qフラグは照会モード、ちなみにeは編集モード
    ...
    for dst in transferedMesh:
        ...
        # スキンクラスタの有無を取得
        dstSkinCluster = cmds.ls(
            cmds.listHistory(shapes[0]), type="skinCluster"
        )
        # スキンクラスタがない場合はあらかじめ取得しておいた情報をもとにバインドする
        if not dstSkinCluster:
            # バインド
    ...
    if srcShapes:
        ...
        if srcSkinCluster:
            ...
            if symWeight is False or dstNode is None:
                return
            transfer_weight(
                srcNode, dstNode, transferWeight=False, returnInfluences=True
            )
            dstShapes = cmds.listRelatives(
                dstNode, s=True, pa=True, type="mesh"
            )
            dstSkinCluster = cmds.listConnections(
                dstShapes[0] + ".inMesh", s=True, d=False
            )
            cmds.copySkinWeights(
                ss=srcSkinCluster[0],
                ds=dstSkinCluster[0],
...
def joint_label(object, visibility=False):
    ...
    for skinJoint in object:
        ...
        if objTypeName == "joint":
            ...
            side = 0
            side_name = ""
            for i, (l_list, r_list) in enumerate(
                zip(left_list_list, right_list_list)
            ):
            ...
            # スケルトン名設定
            cmds.setAttr(
                skinJoint + ".otherType", new_joint_name, type="string"
            )
            # 可視性設定
...
def toggle_mute_skinning():
    ...
    for node in objects:
        children = cmds.ls(
            cmds.listRelatives(node, ad=True, f=True), type="transform"
        )
