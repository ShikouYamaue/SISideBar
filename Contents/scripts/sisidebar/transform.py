...
def move_center2selection():
    ...
    if cmds.selectMode(q=True, co=True):
        ...
        for i in range(3):
            center[i] = sum(
                [cmds.pointPosition(vtx, w=True)[i] for vtx in verticies]
            ) / len(verticies)
        # print center
    ...
    childeNodes = common.search_polygon_mesh(
        selection, serchChildeNode=True, fullPath=True, nurbs=True
    )
    # print childeNodes
...
def reset_actor():
    ...
    for s in sel:
        if cmds.nodeType(s) == "KTG_ModelRoot":
            child_joints = cmds.ls(
                cmds.listRelatives(s, ad=True, f=True), l=True, type="joint"
            )
            if child_joints:
                joints += child_joints
    if not sel:
    ...
    for sel in selections:
        ...
        if c_comp:
            common.TemporaryReparent().main(
                sel, dummyParent=dummy, mode="parent"
            )
...
def freeze_transform(mode="", c_comp=False):
    ...
    for sel in sel_depth:
        ...
        try:
            if mode == "all":
                cmds.makeIdentity(
                    sel, n=0, s=1, r=1, jointOrient=1, t=1, apply=True, pn=1
                )
            ...
            if mode == "trans":
                cmds.makeIdentity(
                    sel, n=0, s=0, r=0, jointOrient=0, t=1, apply=True, pn=1
                )
                cmds.xform(srt_dummy, t=[0, 0, 0])
            if mode == "rot":
                cmds.makeIdentity(
                    sel, n=0, s=0, r=1, jointOrient=0, t=0, apply=True, pn=1
                )
                cmds.xform(srt_dummy, ro=[0, 0, 0])
            if mode == "scale":
                cmds.makeIdentity(
                    sel, n=0, s=1, r=0, jointOrient=0, t=0, apply=True, pn=1
                )
                cmds.xform(srt_dummy, s=[1, 1, 1])
            if mode == "joint":
            ...
            print(e.message)
            cmds.inViewMessage(
                amg=e.message, pos="midCenterTop", fade=True, ta=0.75, a=0.5
            )
        set_attr_locked(sel, attr_lock_flag_list)
        common.TemporaryReparent().main(sel, dummyParent=dummy, mode="parent")
        common.TemporaryReparent().main(
            sel, dummyParent=srt_dummy, mode="parent"
        )
        common.TemporaryReparent().main(dummyParent=dummy, mode="delete")
        common.TemporaryReparent().main(
            dummyParent=srt_dummy, mode="delete"
        )  # ダミー親削除
    cmds.select(selections, r=True)
    sisidebar_sub.get_matrix()
...
def match_transform(mode="", child_comp=False):
    ...
    set_maching(
        nodes=selection, mode=mode, pre_sel=pre_sel, child_comp=child_comp
    )

    msg = lang.Lang(
        en="<hl>Select Matching Object</hl>", ja="<hl>一致対象オブジェクトを選択してください</hl>"
    )
    cmds.inViewMessage(amg=msg.output(), pos="midCenterTop", fade=True)
    # cmds.select(cl=True)
    maching_tool = cmds.scriptCtx(
        title="Much Transform",
    ...
    jobNum = cmds.scriptJob(
        ro=True,
        e=("SelectionChanged", qt.Callback(trs_matching)),
        protected=True,
    )
    sisidebar_sub.get_matrix()
...
def trs_matching(node=None, sel_org=True):
    ...
    for obj in matching_obj:
        ...
        if mode == "rotate" or mode == "all":
            cmds.rotate(
                rot[0], rot[1], rot[2], obj, ws=True, pcp=child_comp_flag
            )
        if mode == "translate" or mode == "all":
            cmds.move(
                pos[0], pos[1], pos[2], obj, ws=True, pcp=child_comp_flag
            )
    if sel_org:
        finish_matching()
