...
def maya_export():
    ...
    if selection:
        cmds.inViewMessage(
            amg="<hl>Go Maya</hl> : Export Selected Objects",
            pos="midCenterTop",
            fade=True,
            ta=0.75,
            a=0.5,
...
def maya_import():
    temp = __name__.split(".")  # nameは自分自身のモジュール名。splitでピリオドごとに3分割。
    folderPath = os.path.join(
        os.getenv("MAYA_APP_DIR"), "Scripting_Files", "go"
    )
    if not os.path.exists(folderPath):
        os.makedirs(os.path.dirname(folderPath + "\\"))  # 末尾\\が必要なので注意
    # print folderPath
    ...
    if files is not None:
        ...
        cmds.inViewMessage(
            amg="<hl>Go Maya</hl> : Imoprt objects",
            pos="midCenterTop",
            fade=True,
            ta=0.75,
            a=0.5,
        ...
        cmds.inViewMessage(
            amg="<hl>Go Maya</hl> : There is no exported object",
            pos="midCenterTop",
            fade=True,
            ta=0.75,
            a=0.5,
