...
class FloatingWindow(qt.SubWindow):
    def __init__(self, parent=None, menus=[], offset=None, menu_name='transform_top'):
        ...
        qt.change_button_color(self, textColor=menu_text, bgColor=menu_bg,
                               hiText=menu_high_text, hiBg=menu_high_bg, mode='window')
...
def move_to_best_pos(object=None, offset=None, move_obj=True):
    ...
    if maya_ver >= 2017:
        float_flag = cmds.workspaceControl(
            u'SiSideBarWorkspaceControl', q=True, fl=True)
    else:
        if maya_ver >= 2015:
...
class SiSideBarWeight(qt.DockWindow):
    ...
    def select_from_current_context(self, select_handle=True):
        ...
        tools_list = ['scaleSuperContext', 'RotateSuperContext',
                      'moveSuperContext', 'selectSuperContext']
        try:
    ...
    def toggle_xyz_icon(self, but=None, axis=0):
        icon_list = [[self.x_on, self.x_off], [
            self.y_on, self.y_off], [self.z_on, self.z_off]]
        if but.isChecked():
    ...
    def select_xyz_from_manip(self, handle_id=0, keep=True):
        try:
            current_tool = cmds.currentCtx()
            tools_list = ['scaleSuperContext',
                          'RotateSuperContext', 'moveSuperContext']
            try:
                mode = tools_list.index(current_tool)
            except:
                return
        ...
        try:
            if cmds.selectMode(q=True, o=True):
                ...
                if sel:
                    ...
                    type = None
                if maya_ver >= 2015:
                    cmds.manipScaleContext('Scale', e=True,
                                           prd=(lambda: set_child_comp(
                                               mode=True), type),  # ドラッグ前に実行
                                           # ドラッグ後に実行
                                           pod=(self.editing_manip, type),
                                           prc=(self.select_from_current_context))  # ツールを開始したときに実行
                    cmds.manipRotateContext('Rotate', e=True,
                                            prd=(lambda: set_child_comp(
                                                mode=True), type),  # ドラッグ前に実行
                                            # ドラッグ後に実行
                                            pod=(self.editing_manip, type),
                                            prc=(self.select_from_current_context))  # ツールを開始したときに実行
                    cmds.manipMoveContext('Move', e=True,
                                          prd=(lambda: set_child_comp(
                                              mode=True), type),  # ドラッグ前に実行
                                          # ドラッグ後に実行
                                          pod=(self.editing_manip, type),
            ...
            return
        target_tool_list = ['scaleSuperContext', 'RotateSuperContext',
                            'moveSuperContext', 'selectSuperContext']
    ...
    def init_save(self):
        ...
        self.dir_path = os.path.join(
            os.getenv('MAYA_APP_dir'),
            'Scripting_Files')
        self.w_file = self.dir_path+'/' + \
            temp[-1]+'_window_'+str(maya_ver)+'.json'
    ...
    trs_attr_list = ['.scaleX', '.scaleY', '.scaleZ', '.rotateX',
                     '.rotateY', '.rotateZ', '.translateX', '.translateY', '.translateZ']
    ...
    def create_fcurve_job(self):
        ...
        for node, attr in itertools.product(selection, self.trs_attr_list):
            ...
            if not cmds.nodeType(fcurve) in anim_curve_list:
                continue
            job = cmds.scriptJob(attributeChange=[
                                 fcurve[0]+'.outStippleRange',  self.check_key_anim_from_fcurve])
            self.fcurve_job_list.append(job)
            job = cmds.scriptJob(
                attributeChange=[fcurve[0]+'.apply', self.check_key_anim_from_fcurve])
            self.fcurve_job_list.append(job)
    ...
    def create_job(self):
        ...
        script_job = cmds.scriptJob(cu=True, e=(
            "SelectionChanged", sisidebar_sub.change_selection))
        timeline_job = cmds.scriptJob(cu=True, e=(
            "timeChanged", sisidebar_sub.change_selection))
        undo_job = cmds.scriptJob(cu=True, e=(
            "Undo", sisidebar_sub.change_selection))
        redo_job = cmds.scriptJob(cu=True, e=(
            "Redo", sisidebar_sub.change_selection))
        context_job = cmds.scriptJob(cu=True, e=(
            "ToolChanged", sisidebar_sub.change_context))
        workspace_job = cmds.scriptJob(
            e=("SceneOpened", setup.check_open), kws=False)
        fcurve_job = cmds.scriptJob(cu=True, e=(
            "SelectionChanged", self.create_fcurve_job))
        # ジョブ番号確認用
        ...
        job_list = [script_job, context_job, timeline_job,
                    undo_job, redo_job, workspace_job, fcurve_job]
    ...
    def remove_job(self):
        ...
        try:
            ...
            # print 'remove attr job error :', e.message
            pass
        try:
            self.kill_fcurve_job()
        except Exception as e:
                ...
                try:
                    ...
                    if any(xyz_list):
                        ...
                        if cmds.selectMode(q=True, o=True):
                            ...
                            space_group.button(
                                self.scl_obj_space).setChecked(True)
                        ...
                        if cmds.selectMode(q=True, co=True):
                            ...
                            space_group.button(
                                self.scl_cmp_space).setChecked(True)
    ...
    def _initUI(self, color_only=False):
        ...
        vn += 1
        # カラー変更ボタン--------------------------------------------------------------------------------
        self.ui_si = make_flat_button(icon=image_path+'SI_Icon.png', name=' S_Color ',
                                      text=text_col, bg=hilite, h_max=22, w_min=si_w, w_max=si_w)
        self.main_layout.addWidget(self.ui_si, vn, 0, 1, 6)
        self.ui_maya = make_flat_button(icon=image_path+'Maya_Icon.png', name='M_Color ',
                                        text=text_col, bg=hilite, h_max=22, w_min=maya_w, w_max=maya_w)
        ...
        self.main_layout.addWidget(make_h_line(
            text=line_col, bg=line_col), vn, 0, 1, 11)
        # --------------------------------------------------------------------------------
        vn += 1
        self.select_top = make_flat_button(
            name=u'▽ Select  ', checkable=False, flat=False, text=text_col, h_min=top_h, bg=mid_color, hover=top_hover)
        self.select_top.clicked.connect(lambda: self.pop_top_menus(
        ...
        global select_but
        select_but = make_flat_button(name='', text=text_col, bg=ui_color, icon_size=(
            44, 44), policy=True, h_max=None, hover=False)
        ...
        select_group_but.toggled.connect(
            lambda: sisidebar_sub.change_group_mode(select_group_but.isChecked()))
        select_group_but.toggled.connect(
        ...
        center_mode_but.toggled.connect(lambda: toggle_center_mode(
        ...
        self.select_Marker_but = make_flat_button(
            icon=':/pickHandlesObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.select_joint_but = make_flat_button(
            icon=':/pickJointObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.select_curve_but = make_flat_button(
            icon=':/pickCurveObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.select_surface_but = make_flat_button(
            icon=':/pickGeometryObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.select_deform_but = make_flat_button(
            icon=':/pickDeformerObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.select_other_but = make_flat_button(
            icon=':/pickOtherObj.png', name='', text=text_col, bg=hilite, w_max=filter_w, h_max=filter_h, tip=tip)
        ...
        self.pick_up = make_flat_button(icon=':/arrowUp', name='', text=text_col,
                                        bg=ui_color, checkable=False, w_max=ud_w, h_max=ud_h, h_min=None)
        ...
        self.pick_down = make_flat_button(
            icon=':/arrowDown', name='', text=text_col, bg=ui_color, checkable=False, w_max=ud_w, h_max=ud_h, h_min=None)
        self.pick_down.clicked.connect(lambda: self.pick_walk(mode='down'))
        self.main_layout.addWidget(self.pick_down, vn+1, 8, 1, 3)
        self.pick_left = make_flat_button(icon=':/arrowLeft', name='', text=text_col,
                                          bg=ui_color, checkable=False, w_max=lr_w, h_max=lr_h, h_min=lr_min)
        ...
        self.pick_right = make_flat_button(
            icon=':/arrowRight', name='', text=text_col, bg=ui_color, checkable=False, w_max=lr_w, h_max=lr_h, h_min=lr_min)
        ...
        self.all_filter = make_flat_button(
            name='All', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from all node types')
        ...
        self.transform_filter = make_flat_button(
            name='Trs', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Transform node')
        self.transform_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.transform_filter.text()))
        self.main_layout.addWidget(self.transform_filter, vn, vh, 1, hw)
        vh += hw
        self.joint_filter = make_flat_button(
            name='Jot', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Joint')
        ...
        self.shape_filter = make_flat_button(
            name='Sap', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Shape node')
        ...
        self.dummy_but_a = make_flat_button(
            name='Nan', text=mute_text, bg=hilite, checkable=False, w_max=fw, h_max=fh, tip='Future filters will be added')
        ...
        self.parent_cons_filter = make_flat_button(
            name='Pac', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Parent Constraint')
        self.parent_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.parent_cons_filter.text()))
        self.main_layout.addWidget(self.parent_cons_filter, vn, vh, 1, hw)
        vh += hw
        self.point_cons_filter = make_flat_button(
            name='Poc', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Point Constraint')
        self.point_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.point_cons_filter.text()))
        self.main_layout.addWidget(self.point_cons_filter, vn, vh, 1, hw)
        vh += hw
        self.orient_cons_filter = make_flat_button(
            name='Orc', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Orient Constraint')
        self.orient_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.orient_cons_filter.text()))
        self.main_layout.addWidget(self.orient_cons_filter, vn, vh, 1, hw)
        vh += hw
        self.scale_cons_filter = make_flat_button(
            name='Slc', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Scale Constraint')
        self.scale_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.scale_cons_filter.text()))
        self.main_layout.addWidget(self.scale_cons_filter, vn, vh, 1, hw)
        vh += hw
        self.aim_cons_filter = make_flat_button(
            name='Aic', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Aim Constraint')
        self.aim_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.aim_cons_filter.text()))
        ...
        # vn+=1
        # 一括操作ようにリストにしておく
        self.all_select_but_list = [select_but,
                                    select_group_but, center_mode_but]
        ...
        self.all_search_widgets = [self.selection_line, self.index_line,
                                   self.pick_down, self.pick_left, self.pick_up, self.pick_right]
        ...
        self.select_section_but = self.all_select_but_list+self.all_filter_but_list + \
            self.all_search_widgets+self.filter_but_list+self.select_lines
        # 高さを保存
        self.select_section_height = [but.height()
                                      for but in self.select_section_but]
        self.select_top.rightClicked.connect(lambda: self.toggle_ui(
            buttons=self.select_section_but,  heights=self.select_section_height))
        # --------------------------------------------------------------------------------
        # トランスフォームエリア
        # action.triggered.connect()
        self.transform_top = make_flat_button(
            name=u'▽ Transform  ', checkable=False, flat=False, text=text_col, h_min=top_h, bg=mid_color, hover=top_hover)
        self.transform_top.clicked.connect(lambda: self.pop_top_menus(
        ...
        scale_x.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=scale_x.text(), current=(0, 0))))
        scale_x.textChanged.connect(lambda: self.keep_pre_line_text(
            text=scale_x.text(), current=(0, 0)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        scale_x.editingFinished.connect(qt.Callback(
            lambda: self.scaling(text=scale_x.text(), axis=0)))
        ...
        scale_y.editingFinished.connect(qt.Callback(lambda: self.check_multi_selection(
            text=scale_y.text(), current=(0, 1))))  # マルチラインは先にコネクト
        scale_y.textChanged.connect(lambda: self.keep_pre_line_text(
            text=scale_y.text(), current=(0, 1)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        scale_y.editingFinished.connect(qt.Callback(
            lambda: self.scaling(text=scale_y.text(), axis=1)))
        ...
        scale_z.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=scale_z.text(), current=(0, 2))))
        scale_z.textChanged.connect(lambda: self.keep_pre_line_text(
            text=scale_z.text(), current=(0, 2)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        scale_z.editingFinished.connect(qt.Callback(
            lambda: self.scaling(text=scale_z.text(), axis=2)))
        ...
        self.but_scale_all = make_flat_button(
            icon=image_path+self.all_axis_icon, name='', text=text_col, bg=hilite, w_max=sel_w, h_max=sel_h)
        ...
        key_rot_x.rightClicked.connect(qt.Callback(
            lambda: set_key_frame(mode=1, axis=3)))
        self.main_layout.addWidget(key_rot_x, vn, tw, 1, anim_b)
        tw += anim_b
        rot_x = self.make_line_edit(text=string_col, bg=bg_col)
        rot_x.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=rot_x.text(), current=(1, 0))))
        rot_x.textChanged.connect(lambda: self.keep_pre_line_text(
            text=rot_x.text(), current=(1, 0)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        rot_x.editingFinished.connect(qt.Callback(
            lambda: self.rotation(text=rot_x.text(), axis=0)))
        ...
        key_rot_y.rightClicked.connect(qt.Callback(
            lambda: set_key_frame(mode=1, axis=3)))
        self.main_layout.addWidget(key_rot_y, vn, tw, 1, anim_b)
        tw += anim_b
        rot_y = self.make_line_edit(text=string_col, bg=bg_col)
        rot_y.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=rot_y.text(), current=(1, 1))))
        rot_y.textChanged.connect(lambda: self.keep_pre_line_text(
            text=rot_y.text(), current=(1, 1)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        rot_y.editingFinished.connect(qt.Callback(
            lambda: self.rotation(text=rot_y.text(), axis=1)))
        ...
        key_rot_z.rightClicked.connect(qt.Callback(
            lambda: set_key_frame(mode=1, axis=3)))
        ...
        rot_z.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=rot_z.text(), current=(1, 2))))
        rot_z.textChanged.connect(lambda: self.keep_pre_line_text(
            text=rot_z.text(), current=(1, 2)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        rot_z.editingFinished.connect(qt.Callback(
            lambda: self.rotation(text=rot_z.text(), axis=2)))
        ...
        trans_x.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=trans_x.text(), current=(2, 0))))
        trans_x.textChanged.connect(lambda: self.keep_pre_line_text(
            text=trans_x.text(), current=(2, 0)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        trans_x.editingFinished.connect(qt.Callback(
            lambda: self.translation(text=trans_x.text(), axis=0)))
        ...
        trans_y.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=trans_y.text(), current=(2, 1))))
        trans_y.textChanged.connect(lambda: self.keep_pre_line_text(
            text=trans_y.text(), current=(2, 1)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        trans_y.editingFinished.connect(qt.Callback(
            lambda: self.translation(text=trans_y.text(), axis=1)))
        ...
        trans_z.editingFinished.connect(qt.Callback(
            lambda: self.check_multi_selection(text=trans_z.text(), current=(2, 2))))
        trans_z.textChanged.connect(lambda: self.keep_pre_line_text(
            text=trans_z.text(), current=(2, 2)))  # 入力変更が成されたかどうかを判定するように即時保存を実行
        trans_z.editingFinished.connect(qt.Callback(
            lambda: self.translation(text=trans_z.text(), axis=2)))
        ...
        self.cog_but.clicked.connect(qt.Callback(
            lambda: self.setup_object_center(change_mode=True)))
        ...
        # vn+=1
        # SRTボタンをまとめてリスト化
        self.scale_but_list = [self.but_scale_x, self.but_scale_y, self.but_scale_z,
                               select_scale, self.but_scale_all, self.lock_attribute_scale]
        self.rot_but_list = [self.but_rot_x, self.but_rot_y, self.but_rot_z,
                             select_rot, self.but_rot_all, self.lock_attribute_rot]
        self.trans_but_list = [self.but_trans_x, self.but_trans_y, self.but_trans_z,
                               select_trans, self.but_trans_all, self.lock_attribute_trans]
        self.all_axis_but_list = [self.scale_but_list,
                                  self.rot_but_list, self.trans_but_list]
        self.key_buts = [key_scale_x, key_scale_y, key_scale_z, key_rot_x,
                         key_rot_y, key_rot_z, key_trans_x, key_trans_y, key_trans_z]
        ...
        # 開閉コネクト
        self.trs_section_height = [but.height()
                                   for but in self.trs_section_widgets]
        self.transform_top.rightClicked.connect(lambda: self.toggle_ui(
            buttons=self.trs_section_widgets,  heights=self.trs_section_height))
        ...
        self.snap_top = make_flat_button(
            name='Snap', checkable=False, flat=False, text=text_col, h_min=top_h, bg=mid_color, hover=top_hover)
        ...
        self.snap_glid_but = make_flat_button(
            icon=':/snapGrid.png', name='', text=text_col, bg=hilite, checkable=True, w_max=snap_w, h_max=snap_h, tip=tip.output())
        ...
        self.snap_segment_but = make_flat_button(
            icon=':/snapCurve.png', name='', text=text_col, bg=hilite, checkable=True, w_max=snap_w, h_max=snap_h, tip=tip.output())
        ...
        self.snap_point_but = make_flat_button(
            icon=':/snapPoint.png', name='', text=text_col, bg=hilite, checkable=True, w_max=snap_w, h_max=snap_h, tip=tip.output())
        ...
        self.snap_pcenter_but = make_flat_button(
            icon=':/snapMeshCenter.png', name='', text=text_col, bg=hilite, checkable=True, w_max=snap_w, h_max=snap_h, tip=tip.output())
        self.snap_pcenter_but.setChecked(snap_mode)
        self.snap_pcenter_but.clicked.connect(lambda: cmds.snapMode(
            meshCenter=self.snap_pcenter_but.isChecked()))
        ...
        self.snap_plane_but = make_flat_button(
            icon=':/snapPlane.png', name='', text=text_col, bg=hilite, checkable=True, w_max=snap_w, h_max=snap_h, tip=tip.output())
        self.snap_plane_but.setChecked(snap_mode)
        self.snap_plane_but.clicked.connect(lambda: cmds.snapMode(
            viewPlane=self.snap_plane_but.isChecked()))
        ...
        self.snap_live_but = make_flat_button(icon=':/makeLiveIcon.png', name='', text=text_col,
                                              bg=hilite, checkable=False, w_max=snap_w, h_max=snap_h, tip=tip.output())
        ...
        self.snap_section_height = [but.height()
                                    for but in self.snap_section_but]
        self.snap_top.rightClicked.connect(lambda: self.toggle_ui(
        ...
        self.constrain_top = make_flat_button(
            name='Constrain', checkable=False, flat=False, text=text_col, h_min=top_h, bg=mid_color, hover=top_hover)
        ...
        tip = lang.Lang(
            en='Toggle Preserve UVs',
            ja=u'UVを保持の切り替え').output()
        ...
        # 設定はシーンのこのトランスフォームの維持を引き継ぐ
        tip = lang.Lang(
            en='Toggle Preserve Children',
            ja=u'子を保持の切り替え').output()
        ...
        # --------------------------------------------------------------------------------
        tip = lang.Lang(
            en='Toggle Tweak Mode',
            ja=u'微調整モードの切り替え').output()
        ...
        # --------------------------------------------------------------------------------
        # 一括操作のためにボタンをリスト化
        self.const_section_but = [self.parent_but, self.cut_but, self.destroy_but,
                                  self.child_comp_but, self.tweak_mode_but, self.keep_uv_but]
        self.const_section_height = [but.height()
                                     for but in self.const_section_but]
        self.constrain_top.rightClicked.connect(lambda: self.toggle_ui(
            buttons=self.const_section_but,  heights=self.const_section_height))
        # --------------------------------------------------------------------------------
        self.main_layout.addWidget(self.make_ds_line(), vn, 0, 1, 11)
        vn += 1
        # エディットエリア
        self.edit_top = make_flat_button(
            name=u'▽ Edit', checkable=False, flat=False, text=text_col, h_min=top_h, bg=mid_color, hover=top_hover)
        self.edit_top.clicked.connect(lambda: self.pop_top_menus(
        ...
        self.plus_but = make_flat_button(
            name='+', text=text_col, bg=hilite, checkable=False, h_max=13, w_max=15, h_min=None)
        ...
        # vn+=1
        # 一括操作のためにボタンをリスト化
        self.edit_section_but = [self.freeze_but, self.group_but,
                                 self.plus_but, self.minus_but, self.freeze_m_but, self.immed_but]
        self.edit_section_height = [but.height()
                                    for but in self.edit_section_but]
        self.edit_top.rightClicked.connect(lambda: self.toggle_ui(
        ...
        qt.change_button_color(self.culc_time_line,
                               textColor=menu_text, bgColor=ui_color)
        ...
        self.numpy = make_flat_button(
            name='Numpy', text=text_col, bg=hilite, h_max=12, w_min=si_w, w_max=si_w)
        #self.main_layout.addWidget(self.numpy, vn, 0, 1 ,6)
        self.standard = make_flat_button(
            name='Standard', text=text_col, bg=hilite, h_max=12, w_min=maya_w, w_max=maya_w)
        ...
        self.all_srt_but_list = [self.scale_but_list,
                                 self.rot_but_list, self.trans_but_list]
        ...
        self.all_multi_list = [self.multi_s_list,
                               self.multi_r_list, self.multi_t_list]
        ...
        self.space_list = [', ws=True', ', ls=True',
                           ', ls=True', ', os=True', ', ws=True', ', ls=True']
    ...
    def destroy_mode(self, init_ui=True):
        ...
        # self.change_ds_line()
        # print msg
        cmds.inViewMessage(amg=msg, pos='midCenterTop',
                           fade=True, ta=0.75, a=0.5)
        # print 'destroy flag :', destroy_flag
        if destroy_flag:
            self.set_ui_color()
            self.collapse_list = [self.select_section_but, self.trs_section_widgets,
                                  self.snap_section_but, self.const_section_but, self.edit_section_but][::-1]
            self.height_list = [self.select_section_height, self.trs_section_height,
                                self.snap_section_height, self.const_section_height, self.edit_section_height][::-1]
    ...
    def destroy_collapse(self):
        ...
        if self.c_count == 5:
            ...
            self.toggle_ui(
                buttons=self.collapse_list[self.c_count],  heights=self.height_list[self.c_count])
        self.c_count += 1
    ...
    def destroy_blinking(self):
        ...
        for but, pt in zip(all_flat_buttons, all_flat_button_palams):
            check = but.isChecked()
            qt.change_button_color(but, textColor=pt[0], bgColor=pt[1], hiColor=pt[2],
                                   mode=pt[3], hover=pt[4], destroy=pt[5], dsColor=bk_bd_col, toggle=True)
        ...
        if self.blinking_count > 500 and self.blinking_times == 0:
            ...
            for but, pt in zip(all_flat_buttons, all_flat_button_palams):
                check = but.isChecked()
                qt.change_button_color(but, textColor=pt[0], bgColor=pt[1], hiColor=pt[2],
                                       mode=pt[3], hover=pt[4], destroy=pt[5], dsColor=border_col, toggle=True)
        # print 'test'
    ...
    def setup_object_center(self, change_mode=False):
        ...
        try:
            if self.cog_but.text() == 'COG':
                ...
                self.spiv_list = [
                    pm.xform(s+'.scalePivot', q=True, t=True, os=True) for s in sel_obj]
                self.rpiv_list = [
                    pm.xform(s+'.rotatePivot', q=True, t=True, os=True) for s in sel_obj]
                if not sel_obj:
                    return
                '''
                global np_flag
        ...
        if self.np_group.checkedId() == 0:
            if not np_exist:
                ...
                cmds.inViewMessage(amg='<hl>Numpy module does not exist.</hl>\nPlease install Numpy.',
                                   pos='midCenterTop', fade=True, ta=0.75, a=0.5)
                self.np_group.button(1).setChecked(True)
                return
    ...
    def load_filter_but(self):
        save_file = self.dir_path + \
            '\\sisidebar_selection_filter_'+str(maya_ver)+'.json'
        ...
        if all_flags[0] == True:
            ...
            for filter_type, flag in zip(self.select_type_list[1:], all_flags[1:]):
                if not flag:
                    continue
                # print filter_type
                selection_node += cmds.ls(search_list,
                                          l=True, type=filter_type)
    ...
    def display_selection(self):
        try:
            ...
            if cmds.selectMode(q=True, co=True):
                ...
                obj_list = list(set([comp.split('.')[0]
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.Wheel:
            ...
            self.culc_input_event(obj=obj, delta=delta,
                                  mod=key_mod, value=value)
        if event.type() == QEvent.FocusIn:
    ...
    def mouse_vector(self):
        ...
        if self.count <= threshold:
            ...
            if self.cur_pos != self.pre_pos:
                ...
                # 現在のベクトルを求める
                self.cur_vec = [self.cur_pos[0]-self.pre_pos[0],
    ...
    def show_ref_menu(self):
        ...
        ref_menus = QMenu()
        qt.change_button_color(ref_menus, textColor=menu_text, bgColor=menu_bg,
                               hiText=menu_high_text, hiBg=menu_high_bg, mode='window')
    ...
    def pick_reference(self, mode=''):
        ...
        if mode == 'object':
            ...
            cmds.selectMode(co=True)
            if mode == 'vertex':
                cmds.selectType(pv=1, smu=0, smp=1, pf=0,
                                pe=0, smf=0, sme=0, puv=0)
            if mode == 'edge':
                cmds.selectType(pv=0, smu=0, smp=0, pf=0,
                                pe=1, smf=0, sme=1, puv=0)
            if mode == 'face':
                cmds.selectType(pv=0, smu=0, smp=0, pf=1,
                                pe=0, smf=0, sme=1, puv=0)
        cmds.select(cl=True)
        ...
        ref_job = cmds.scriptJob(ro=True, ct=[
                                 "SomethingSelected", lambda: sisidebar_sub.set_reference(mode=mode)], protected=True)
    ...
    def create_edit_menu(self, add_float=True):
        ...
        qt.change_button_color(self.edit_menus, textColor=menu_text, bgColor=menu_bg,
                               hiText=menu_high_text, hiBg=menu_high_bg, mode='window')
        ...
        action04.triggered.connect(qt.Callback(
        ...
        action05.triggered.connect(qt.Callback(
            lambda: uv.EditUVSet().main(delMultiUV=False, groupMultiUV=True)))
    ...
    def create_select_menu(self, add_float=True):
        ...
        qt.change_button_color(self.select_menus, textColor=menu_text, bgColor=menu_bg,
                               hiText=menu_high_text, hiBg=menu_high_bg, mode='window')
        if add_float:  # 切り離しウィンドウメニュー
        ...
        self.sch_buts = [self.sel_action00,
                         self.sel_action01, self.sel_action02]
    ...
    def save_axis_select_setting(self):
        self.axis_select_setting_path = self.dir_path + \
            '\\sisidebar_axis_operation_'+str(maya_ver)+'.json'
        if not os.path.exists(self.dir_path):
            os.makedirs(self.dir_path)
        save_data = {}
    ...
    def save_mouse_setting(self):
        self.mouse_setting_path = self.dir_path + \
            '\\sisidebar_mouse_data_'+str(maya_ver)+'.json'
        global right_click_gesture
        global center_click_gesture
        global left_click_gesture
        if not os.path.exists(self.dir_path):
    ...
    def create_f_trans_menu(self):  # ウィンドウ切り離しの場合はインスタンスを別にして再作成
        ...
        transform_manu_window = FloatingWindow(
            menus=top_f_menus, offset=transform_offset, menu_name='transform_top')
        global trs_window_flag
        trs_window_flag = True

    def create_trans_menu(self, add_float=True):
        self.trans_menus = QMenu(self.transform_top)
        qt.change_button_color(self.trans_menus, textColor=menu_text, bgColor=menu_bg,
                               hiText=menu_high_text, hiBg=menu_high_bg, mode='window')
        if add_float:  # 切り離しウィンドウメニュー
        ...
        mag = lang.Lang(en='Transform Preference...',
                        ja=u'変換設定')
        action25 = QAction(mag.output(), self.trans_menus,
                           icon=QIcon(image_path+'setting'))
        ...
        action17.triggered.connect(qt.Callback(
            lambda: transform.set_joint_orient(reset=True)))
        mag = lang.Lang(en='Transfer Joint Orient to Rotate',
                        ja=u'ジョイントの方向を回転に変換')
        action18 = self.trans_menus.addAction(mag.output())
        action18.triggered.connect(qt.Callback(
            lambda: transform.set_joint_orient(reset=False)))
        self.trans_menus.addSeparator()  # 分割線追加
        # ----------------------------------------------------------------------------------------------------
        ...
        action26.triggered.connect(qt.Callback(
        ...
        action28.triggered.connect(qt.Callback(
        ...
        action27.triggered.connect(qt.Callback(
            lambda: toggle_center_mode(mode=False, ntpose=True)))
        self.trans_menus.addSeparator()  # 分割線追加
        # ----------------------------------------------------------------------------------------------------
        mag = lang.Lang(en='Reset All Transforms(with Pivot)',
                        ja=u'すべての変換をリセット(ピボットもリセット)')
        action13 = self.trans_menus.addAction(mag.output())
        action13.triggered.connect(qt.Callback(lambda: transform.reset_transform(
        ...
        action14.triggered.connect(qt.Callback(lambda: transform.reset_transform(
        ...
        action15.triggered.connect(qt.Callback(lambda: transform.reset_transform(
        ...
        action16.triggered.connect(qt.Callback(lambda: transform.reset_transform(
        ...
        action0.triggered.connect(qt.Callback(lambda: transform.freeze_transform(
            mode='all', c_comp=self.child_comp_but.isChecked())))
        mag = lang.Lang(en='Freeze Scaling',
                        ja=u'スケーリングのフリーズ')
        action1 = self.trans_menus.addAction(mag.output())
        action1.triggered.connect(qt.Callback(lambda: transform.freeze_transform(
            mode='scale', c_comp=self.child_comp_but.isChecked())))
        mag = lang.Lang(en='Freeze Rotation',
                        ja=u'回転のフリーズ')
        action2 = self.trans_menus.addAction(mag.output())
        action2.triggered.connect(qt.Callback(lambda: transform.freeze_transform(
            mode='rot', c_comp=self.child_comp_but.isChecked())))
        mag = lang.Lang(en='Freeze Translation',
                        ja=u'移動のフリーズ')
        action3 = self.trans_menus.addAction(mag.output())
        action3.triggered.connect(qt.Callback(lambda: transform.freeze_transform(
            mode='trans', c_comp=self.child_comp_but.isChecked())))
        mag = lang.Lang(en='Freeze Joint Orientation',
                        ja=u'ジョイントの方向のフリーズ')
        action4 = self.trans_menus.addAction(mag.output())
        action4.triggered.connect(qt.Callback(lambda: transform.freeze_transform(
        ...
        self.action20.triggered.connect(qt.Callback(
            lambda: transform.round_transform(mode='all', digit=round_decimal_value)))
        ...
        self.action21.triggered.connect(qt.Callback(
            lambda: transform.round_transform(mode='scale', digit=round_decimal_value)))
        ...
        self.action22.triggered.connect(qt.Callback(
            lambda: transform.round_transform(mode='rotate', digit=round_decimal_value)))
        ...
        self.action23.triggered.connect(qt.Callback(
            lambda: transform.round_transform(mode='translate', digit=round_decimal_value)))
        ...
        self.action24.triggered.connect(qt.Callback(lambda: transform.round_transform(
            mode='jointOrient', digit=round_decimal_value)))
        self.round_action_list = [
            self.action20, self.action21, self.action22, self.action23, self.action24]
        self.trans_menus.addSeparator()  # 分割線追加
        # ----------------------------------------------------------------------------------------------------
        mag = lang.Lang(en='Match All Transform',
                        ja=u'すべての変換の一致')
        action6 = self.trans_menus.addAction(mag.output())
        action6.triggered.connect(lambda: transform.match_transform(
        ...
        action7.triggered.connect(lambda: transform.match_transform(
        ...
        action8.triggered.connect(lambda: transform.match_transform(
        ...
        action9.triggered.connect(lambda: transform.match_transform(
        ...
        action11.triggered.connect(qt.Callback(
            transform.move_center_each_object))
        self.trans_menus.addSeparator()  # 分割線追加
        # ----------------------------------------------------------------------------------------------------
        self.trs_setting_path = self.dir_path + \
            '\\sisidebar_trs_data_'+str(maya_ver)+'.json'
    ...
    def all_toggle_snapping(self):
        ...
        if flag:
            ...
            check_flag = True
        cmds.snapMode(grid=check_flag, curve=check_flag, point=check_flag,
                      meshCenter=check_flag,  viewPlane=check_flag)
    ...
    def select_filter_mode(self, mode=0):
        ...
        filter_coms = ["Marker", "Joint", "Curve", "Surface",
                       "Deformer", "Dynamic", "Rendering", "Other"]
    ...
    attr_lock_flag_list = [[None, None, None],
                           [None, None, None], [None, None, None]]
    all_attr_list = [['.sx', '.sy', '.sz'], [
        '.rx', '.ry', '.rz'], ['.tx', '.ty', '.tz'], []]
    # @prof.profileFunction()
    ...
    def attribute_lock_state(self, mode=0, check_only=False, axis=None, check_key_flag=True, selection=None):
        ...
        if mode == 3:  # モード3なら全部処理する
            ...
            self.attribute_lock_state(
                mode=0, check_only=True, check_key_flag=False, selection=selection)
            self.attribute_lock_state(
                mode=1, check_only=True, check_key_flag=False, selection=selection)
            self.attribute_lock_state(
                mode=2, check_only=True, check_key_flag=False, selection=selection)
            check_key_anim()
            return
                        ...
                        if lock_flag:
                            if not check_only:
                                self.toggle_lock_attr(
                                    mode=mode, state=False, objects=selection, axis=axis)
                                # check_key_anim()
                                return
        ...
        if pre_scale[axis] != value:
            ...
            if components:
                ...
                if self.cog_but.isChecked():
                    ...
                    if sid == 0 or sid == 4:
                        cmds.scale(
                            add_scale[0], add_scale[1], add_scale[2], r=True, ws=True, p=piv_pos, smn=sym)
                    if sid == 1 or sid == 2 or sid == 5:
                        cmds.scale(
                            add_scale[0], add_scale[1], add_scale[2], r=True, ls=True, p=piv_pos, smn=sym)
                    if sid == 3:
                        cmds.scale(
                            add_scale[0], add_scale[1], add_scale[2],  r=True, os=True, p=piv_pos, smn=sym)
                    ...
                    for mesh, vtx in obj_dict.items():
                        ...
                        if sid == 3:  # オブジェクトモードの時だけそれぞれの角度にスケール
                            ...
                            cmds.scale(
                                add_scale[0], add_scale[1], add_scale[2], r=True, os=True, smn=sym)
                            ...
                            if sid == 0 or sid == 4:
                                cmds.scale(
                                    add_scale[0], add_scale[1], add_scale[2], r=True, ws=True, p=base_pos, smn=sym)
                            if sid == 1 or sid == 2 or sid == 5:
                                cmds.scale(
                                    add_scale[0], add_scale[1], add_scale[2], r=True, ls=True, p=base_pos, smn=sym)
    ...
    def rotation(self, text='', axis=0, focus=True):
        ...
        for sel in transforms:
            ...
            if sid == 1 or sid == 2:  # ローカルスペースとビューの時の処理
                # print 'rot os'
                exec(
                    'cmds.rotate(rot[0], rot[1], rot[2], sel'+pcp+', os=True)')
            else:  # グローバル処理
                exec(
                    'cmds.rotate(rot[0], rot[1], rot[2], sel'+pcp+', ws=True)')
        ...
        if pre_rot[axis] != value:
            ...
            if components:
                ...
                if self.cog_but.isChecked():
                    ...
                    if sid == 0 or sid == 4:
                        cmds.rotate(
                            add_rot[0], add_rot[1], add_rot[2], r=True, ws=True, p=piv_pos, smn=smn)
                    if sid == 3:  # ジンバル
                        cmds.rotate(
                            add_rot[0], add_rot[1], add_rot[2], r=True, eu=True, p=piv_pos, smn=smn)
                    if sid == 1 or sid == 2 or sid == 5:  # オブジェクト
                        cmds.rotate(
                            add_rot[0], add_rot[1], add_rot[2], r=True, os=True, p=piv_pos, smn=smn)
                        # return
                else:
                    # COGグローバル以外の処理
                    if sid == 0 or sid == 4:  # ワールドスペース
                        # print 'global_mode :'
                        cmds.rotate(add_rot[0], add_rot[1],
                                    add_rot[2],  r=True, ws=True, smn=smn)
                    if sid == 3:  # ジンバル
                        # print 'object mode :'
                        cmds.rotate(add_rot[0], add_rot[1],
                                    add_rot[2], r=True, eu=True, smn=smn)
                    if sid == 1 or sid == 2 or sid == 5:  # オブジェクト
                        # print 'local_mode :'
                        cmds.rotate(add_rot[0], add_rot[1],
                                    add_rot[2],  r=True, os=True, smn=smn)
        sisidebar_sub.get_matrix()
        # self.out_focus()
        if focus:
            global input_line_id  # フォーカス外すラインを限定する
                    ...
                    for mesh, vtx in obj_dict.items():
                        ...
                        if sign:
                            if sign == '+':
                                ...
                                exec(
                                    'add_value = pre_trans[axis] '+sign+' value-pre_trans[axis]')
                        else:
                            if cp_abs_flag:
                        ...
                        if sym:
                            ...
                            if sid == 0 or sid == 4:  # ワールドスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2], r=True, smn=smn)
                                #cmds.xform(vtx, t=add_trans, r=True, ws=True)
                            elif sid == 3 or sid == 2 or sid == 5:  # ローカルスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2],  r=True, ls=True, smn=smn)
                                #cmds.xform(vtx, t=add_trans, r=True, os=True)
                            elif sid == 1:  # オブジェクトスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2], r=True, os=True, wd=sym, smn=smn)
                        else:
                            if sid == 0 or sid == 4:  # ワールドスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2], r=True, ws=True)
                            #cmds.xform(vtx, t=add_trans, r=True, ws=True)
                            elif sid == 3 or sid == 2 or sid == 5:  # ローカルスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2],  r=True, ls=True)
                            #cmds.xform(vtx, t=add_trans, r=True, os=True)
                            elif sid == 1:  # オブジェクトスペース
                                cmds.move(
                                    add_trans[0], add_trans[1], add_trans[2], r=True, os=True)
        sisidebar_sub.get_matrix()
        # self.out_focus()
        if focus:
            global input_line_id  # フォーカス外すラインを限定する
    ...
    def exchange_abs_val(self, components, axis, abs):
        pos_list = [cmds.xform(con, q=True, t=True, ws=True)
...
def set_key_frame(mode=0, axis=0, force=None):
    ...
    global key_colors
    if axis == 3:
        all_id = [True if key_colors[mode*3+i]
                  == 3 else False for i in range(3)]
        if all(all_id):
            force = False
        else:
            force = True
...
def transform_center():
    ...
    for sel, center in zip(center_objects, centers):
        ...
        try:
            cmds.makeIdentity(str(sel), n=0, s=0, r=0,
                              jointOrient=0, t=1, apply=True, pn=1)
        except Exception as e:
            print(e.message)
        try:
            cmds.makeIdentity(str(sel), n=0, s=0, r=1,
                              jointOrient=0, t=0, apply=True, pn=1)
        except Exception as e:
            print(e.message)
        try:
            cmds.makeIdentity(str(sel), n=0, s=1, r=0,
                              jointOrient=0, t=0, apply=True, pn=1)
        except Exception as e:
            print(e.message)
        cmds.move(cp[0], cp[1], cp[2], str(sel)+'.scalePivot',
                  str(sel)+'.rotatePivot', ws=True, pcp=True)
        cmds.rotate(cr[0], cr[1], cr[2], str(sel)+'.scalePivot',
                    str(sel)+'.rotatePivot', ws=True, pcp=True)
        cmds.scale(cs[0], cs[1], cs[2], str(sel)+'.scalePivot',
                   str(sel)+'.rotatePivot', os=True, pcp=True)
...
def toggle_center_mode(init=None, mode=None, change=False, ntpose=False, lock=False):
    ...
    if mode:
        ...
        for sel in center_objects:
            ...
            # print center
            cmds.move(p[0], p[1], p[2], str(center), str(
                center)+'.scalePivot', str(center)+'.rotatePivot', ws=True, pcp=True)
    ...
    if not mode:  # センターモード、ニュートラルポーズ解除モード
        ...
        if ntpose:  # ニュートラルポーズのときは親がntノードかどうかを判断して格納する
            center_objects = [sel for sel in pm.ls(
                sl=True, l=True, tr=True) if not str(sel).endswith('_ntpose')]
            centers = [pm.listRelatives(sel, p=True)[0] if pm.listRelatives(
                sel, p=True) else None for sel in center_objects]
        for sel, center in zip(center_objects, centers):
            if ntpose:
                if center is None:
                    continue
...
def check_key_anim(from_fcurve=False):
    ...
    key_buts = [key_scale_x, key_scale_y, key_scale_z, key_rot_x,
                key_rot_y, key_rot_z, key_trans_x, key_trans_y, key_trans_z]
    key_names = ['scaleX', 'scaleY', 'scaleZ',
                 'rotateX', 'rotateY', 'rotateZ',
                 'translateX', 'translateY', 'translateZ']
    icon_list = ['Key_N.png', 'Key_G.png', 'Key_Y.png',
                 'Key_R.png', 'Key_N.png', 'Key_B.png', 'Key_I.png']
    if len(selection) >= 500:
        key_colors = [6]*9
        pass
    else:
...
def set_active_mute(mode=0):
    ...
    text_col_list = [text_col, text_col,
                     text_col, text_col, text_col, text_col]
    sel_text_col_list = [text_col, text_col,
                         text_col, mute_text, text_col, mute_text]
    if cmds.selectMode(q=True, o=True):
    ...
    for i, (button, name) in enumerate(zip(space_but_list, name_list)):
        ...
        try:
            ...
            pass
        button.setText(name)
        qt.change_button_color(button, textColor=color, bgColor=ui_color, hiColor=hilite,
                               mode='button', toggle=True, destroy=destroy_flag, dsColor=border_col)
    ...
    if cmds.selectMode(q=True, o=True):
        ...
        qt.change_button_color(center_mode_but, textColor=text_col, bgColor=ui_color,
                               hiColor=hilite, mode='button', toggle=True, destroy=destroy_flag, dsColor=border_col)
    else:
        select_group_but.setText('Cluster')
        center_mode_but.setDisabled(True)
        center_mode_but.setChecked(False)
        qt.change_button_color(center_mode_but, textColor=mute_text, bgColor=ui_color,
                               mode='button', toggle=True, destroy=destroy_flag, dsColor=border_col)
...
def set_srt_text(scale, rot, trans):
    ...
    trans = map(str, trans)
    # 念のため0のマイナス符号を除去
    scale = map(lambda a: a.replace('-0.0', '0.0')
                if a == '-0.0' else a, scale)
    rot = map(lambda a: a.replace('-0.0', '0.0') if a == '-0.0' else a, rot)
    trans = map(lambda a: a.replace('-0.0', '0.0')
                if a == '-0.0' else a, trans)
    try:
...
class PropOption(qt.MainWindow):
    def __init__(self, parent=None):
        ...
        qt.change_widget_color(
            self.radius, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        ...
        for i, ci in enumerate(curve_icons):
            button = make_flat_button(
                icon=ci, name='', text=text_col, bg=hilite, checkable=True, w_max=24)
...
class SymOption(qt.MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        qt.change_widget_color(off, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        world = QRadioButton(self.msg01.output(), self)
        qt.change_widget_color(world, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        object = QRadioButton(self.msg02.output(), self)
        qt.change_widget_color(object, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        ...
        if maya_ver >= 2015:
            ...
            qt.change_widget_color(topology, textColor=menu_text,  bgColor=ui_color,
                                   baseColor=radio_base_col, windowText=menu_text)
        ...
        qt.change_widget_color(x, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        y = QRadioButton('Y', self)
        qt.change_widget_color(y, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        z = QRadioButton('Z', self)
        qt.change_widget_color(z, textColor=menu_text,  bgColor=ui_color,
                               baseColor=radio_base_col, windowText=menu_text)
        ...
        qt.change_widget_color(
            self.tolerance, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        ...
        self.tolerance_sld.valueChanged[int].connect(
            lambda: self.tolerance.setValue(self.tolerance_sld.value()/100000.0))
        self.tolerance.editingFinished.connect(
            lambda: self.tolerance_sld.setValue(self.tolerance.value()*100000))
        ...
        qt.change_widget_color(self.preserve_seam, textColor=menu_text,
                               bgColor=ui_color, baseColor=radio_base_col)
        ...
        qt.change_widget_color(
            self.seam_tol, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        ...
        self.seam_tol_sld.valueChanged[int].connect(
            lambda: self.seam_tol.setValue(self.seam_tol_sld.value()/100000.0))
...
class TransformSettingOption(qt.MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        qt.change_widget_color(
            self.view_decimal, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        # スライダバーを設定
        ...
        qt.change_widget_color(
            self.round_decimal, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        # スライダバーを設定
        ...
        qt.change_widget_color(
            self.mouse_gesture, textColor=string_col, bgColor=mid_color, baseColor=bg_col)
        # スライダバーを設定
...
class FilterOption(qt.MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        fw = None
        fh = None
        filter_label = make_flat_button(name='- Selection Filter Option -', text=text_col,
                                        bg=hilite, checkable=True, w_max=fw, h_max=20, tip='- Selection Filter Option -')
        self.main_layout.addWidget(filter_label)
        filter_label.setDisabled(True)
        self.main_layout.addWidget(window.make_h_line())

        self.all_filter = make_flat_button(name='All Node Types', text=text_col, bg=hilite,
                                           checkable=True, w_max=fw, h_max=fh, tip='Search from all node types')
        self.all_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.all_filter.text()))
        self.main_layout.addWidget(self.all_filter)
        self.transform_filter = make_flat_button(
            name='Transform node', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Transform node')
        self.transform_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.transform_filter.text()))
        self.main_layout.addWidget(self.transform_filter)
        self.joint_filter = make_flat_button(
            name='Joint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Joint')
        self.joint_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.joint_filter.text()))
        self.main_layout.addWidget(self.joint_filter)
        self.shape_filter = make_flat_button(
            name='Shape node', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Shape node')
        self.shape_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.shape_filter.text()))
        ...
        self.parent_cons_filter = make_flat_button(
            name='Parent Constraint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Parent Constraint')
        self.parent_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.parent_cons_filter.text()))
        self.main_layout.addWidget(self.parent_cons_filter)
        self.point_cons_filter = make_flat_button(
            name='Point Constraint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Point Constraint')
        self.point_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.point_cons_filter.text()))
        self.main_layout.addWidget(self.point_cons_filter)
        self.orient_cons_filter = make_flat_button(
            name='Orient Constraint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Orient Constraint')
        self.orient_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.orient_cons_filter.text()))
        self.main_layout.addWidget(self.orient_cons_filter)
        self.scale_cons_filter = make_flat_button(
            name='Scale Constraint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Scale Constraint')
        self.scale_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.scale_cons_filter.text()))
        self.main_layout.addWidget(self.scale_cons_filter)
        self.aim_cons_filter = make_flat_button(
            name='Aim Constraint', text=text_col, bg=hilite, checkable=True, w_max=fw, h_max=fh, tip='Search from Aim Constraint')
        self.aim_cons_filter.clicked.connect(
            lambda: self.set_filter_but(filter_type=self.aim_cons_filter.text()))
...
class RockAttrMenu(qt.SubWindow):
    def __init__(self, parent=None, mode=0, name='Scale'):
        ...
        button = make_flat_button(name='All Axis', text=menu_text,
                                  bg=menu_bg, costom_push=menu_bg, flat=False, checkable=False)
        button.clicked.connect(lambda: window.attribute_lock_state(mode=mode))
        p_layout.addWidget(button)
        button = make_flat_button(name=name+' X', text=menu_text,
                                  bg=menu_bg, costom_push=menu_bg, flat=False, checkable=False)
        button.clicked.connect(
            lambda: window.attribute_lock_state(mode=mode, axis=0))
        p_layout.addWidget(button)
        button = make_flat_button(name=name+' Y', text=menu_text,
                                  bg=menu_bg, costom_push=menu_bg, flat=False, checkable=False)
        button.clicked.connect(
            lambda: window.attribute_lock_state(mode=mode, axis=1))
        p_layout.addWidget(button)
        button = make_flat_button(name=name+' Z', text=menu_text,
                                  bg=menu_bg, costom_push=menu_bg, flat=False, checkable=False)
        button.clicked.connect(
