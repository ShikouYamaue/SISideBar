...
def face_extraction(
    faces=None, deleteOrg=True, selectDuplicated=True, transferWeight=True
):
    """
    メッシュを選択したフェイスで切り分ける
        ...
        if len(compTemp) != 0:
            ...
            if transferWeight:  # ウェイト転送フラグが有効だったら
                weight.transfer_weight(
                    selObj, dupObj, logTransfer=False
                )  # ウェイトを転送
...
class ClusterCopy:
    def copy(self, mesh):
        ...
        for cls in cluster:
            set_node = cmds.ls(
                cmds.listHistory(cls, f=True), type="objectSet", l=True
            )[0]
        ...
        common.TemporaryReparent().main(
            mesh, dummyParent=dummy, mode="parent"
        )  # コピーのおわったメッシュの子供を元に戻す
        common.TemporaryReparent().main(
            dummyParent=dummy, mode="delete"
        )  # ダミー親削除
...
def duplicate_with_skin(nodes, parentNode=None):
    ...
    for node in nodes:
        ...
        weight.SimpleWeightCopyPaste().main(
            node, mode="copy", saveName=__name__, weightFile=node
        )
        weight.SimpleWeightCopyPaste().main(
            dup, mode="paste", saveName=__name__, weightFile=node
        )
        # 親子付けを戻す
...
def cehck_zero_poly_object(mesh=None, pop_msg=True):
    ...
    if polyMeshes == None:
        if pop_msg:
            cmds.confirmDialog(
                title="Check", message="Zero Polygon Object Count :  0"
            )
        return zeroPolyObj
    for p in polyMeshes:
...
class MeshMarge:
    ...
    def marge_run(self):
        objects = common.search_polygon_mesh(
            self.objects, serchChildeNode=True, fullPath=True
        )
        print(objects)
        # print objects
        if len(objects) < 2:
