...
try:
    MAYA_WIDNOW = shiboken.wrapInstance(
        long(OpenMayaUI.MQtUtil.mainWindow()), QWidget
    )
except:
    MAYA_WIDNOW = None
...
def change_button_color(
    ...
    if mode == "common":
        button.setStyleSheet(
            "color: " + textHex + " ; background-color: " + bgHex
        )
    if mode == "button":
        if not destroy:
            button.setStyleSheet(
