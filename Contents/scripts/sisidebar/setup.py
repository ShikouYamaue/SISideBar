    ...
    for i in range(len(dir_list)):
        ...
        for file in all_files:
            if file == "workspace.mel":
                ...
                return
    cmds.inViewMessage(
        amg=msg01.output(), pos="midCenterTop", fade=True, ta=0.75, a=0.5
    )
...
def set_work_space(root_dir):
    ...
    msg00 = lang.Lang(
        ...
        ja="現在のプロジェクトを<hl>" + current_project + "</hl>に設定しました",
    )
    cmds.inViewMessage(
        amg=msg00.output(), pos="midCenterTop", fade=True, ta=0.75, a=0.5
    )
...
def open_scene(mime_data):
    ...
    for file in url_list:
        ...
        if not ext.lower() in ext_dict.keys():
            continue
        # 保存するかどうか
        msg05 = lang.Lang(en="untitled scene", ja="無題のシーン")
        msg05 = msg05.output()
        scene_path = cmds.file(q=True, sceneName=True)
        if scene_path == "":
            scene_path = msg05
        msg01 = lang.Lang(
            ...
            ja=scene_path + "への変更を保存しますか？",
        )
        msg02 = lang.Lang(en="Save", ja="保存")
        msg03 = lang.Lang(en="Don't Save", ja="保存しない")
        msg04 = lang.Lang(en="Cancel", ja="キャンセル")
        msg01 = msg01.output()
        msg02 = msg02.output()
        msg03 = msg03.output()
        msg04 = msg04.output()
