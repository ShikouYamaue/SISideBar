...
class ExtrudeEdgeUV(qt.SubWindow):
    ...
    ex_edges = []
    save_file = (
        save_path + "\\sisidebar_extrude_setting_" + str(maya_ver) + ".json"
    )
    ...
    def __init__(
        ...
        msg = lang.Lang(
            en="- UV extrusion distance -", ja="- UVの押し出し距離 -"
        ).output()
        ...
        self.d_ratio_sld.valueChanged.connect(
            lambda: self.d_ratio.setValue(self.d_ratio_sld.value() / 100.0)
        )
        self.d_ratio.editingFinished.connect(
            lambda: self.d_ratio_sld.setValue(self.d_ratio.value() * 100)
        )
        ...
        msg = lang.Lang(en="Extrude edges", ja="エッジを押し出し").output()
        self.main_layout.addWidget(qt.make_h_line())
        ...
        msg = lang.Lang(en="Sew UV", ja="UVを縫合").output()
        self.main_layout.addWidget(qt.make_h_line())
        button = qt.make_flat_button(
            name=msg,
            text=text_col,
