...
def main(mesh=None, pop_zero_poly=False):
    ...
    msg01 = lang.Lang(
        en="There is a zero polygon object : ", ja="ゼロポリゴンのメッシュが存在します : "
    )
    msg02 = lang.Lang(
    ...
    for node, node_l in zip(selection, selection_l):
        ...
        meshnode = cmds.listRelatives(
            node_l, s=True, pa=True, type="mesh", fullPath=True
        )
        if meshnode:
...
def store_blend_shape(mesh):
    ...
    if blend_shapes:
        for bs in blend_shapes:
            shape_target = cmds.ls(
                cmds.listConnections(bs, s=True, d=False),
                l=True,
                type="transform",
            )
...
def repareDagSetMember(node):
    ...
    connections = cmds.listConnections(
        shapes[0], d=True, s=False, p=True, c=True
    )
...
def copyWeight(node, engine="maya"):
    weight.WeightCopyPaste().main(
        node, mode="copy", saveName=__name__, engine=engine
    )
...
def freezeModeling(node, engine="maya"):
    ...
    # ウェイトを書き戻してくる
    weight.WeightCopyPaste().main(
        node, mode="paste", saveName=__name__, engine=engine
    )
    # いらないシェイプを消す
...
def deleteZeroShape(node):
    meshnode = cmds.listRelatives(
        node, s=True, pa=True, type="mesh", fullPath=True
    )
...
def freeze():
    ...
    for sel in selection:
        allChildren = [sel] + cmds.listRelatives(
            sel, ad=True
        )  # 子供を取得して1つのリストにする
        ...
        if polyMesh:
            for mesh in polyMesh:
                common.TemporaryReparent().main(
                    mesh, dummyParent=dummy, mode="cut"
                )
